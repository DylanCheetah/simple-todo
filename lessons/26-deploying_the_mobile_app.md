# Lesson 26: Deploying the Mobile App

Up until this point we have been testing our mobile app on a PC. In order to deploy it to a mobile device we will first need to build it. In this lesson we will build our mobile app for use on Android devices. To simplify the process, we will run a Docker container as part of a GitHub Actions workflow to build our mobile app. Before we begin we will first need to create a production configuration for our mobile app. Rename `simple-todo/simple_todo_mobile/config.py` to `config.dbg.py`. Then copy its contents into a new `simple-todo/simple_todo_mobile/config.py` file and change the domain of each URL from "127.0.0.1" to the domain which was assigned to your website by Vercel. Next we need to create `simple-todo/simple_todo_mobile/buildozer.spec` with the following content:
```
...
requirements = python3,kivy,anyio,certifi,h11,httpcore,httpx,idna,typing_extensions
...
Change the value of `package.domain` to the domain for your own organization. Then create `simple-todo/.github/workflows/build.yml` with the following content:
```yaml
name: Build
on:
  push:
jobs:
  build-mobile-app:
    ...
    - name: Build Mobile App
      run: |
        cd simple_todo_mobile
        buildozer android debug
    - name: Upload Artifacts
      uses: actions/upload-artifact@v4
      with:
        name: mobile-app
        path: simple_todo_mobile/bin/*.apk
```

Commit the changes to your GitHub repo and push them. Check the Actions tab of the repository in a web browser to monitor the workflow status. When the workflow completes there should be a build artifact called "mobile-app" which contains a .apk file which you can install on an Android phone.
