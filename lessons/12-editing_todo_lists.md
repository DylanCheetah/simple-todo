...
<div id="todo_list-update-form" class="row justify-content-center">
    <form class="col-8"
          hx-put="{% url 'todo-list' todo_list.pk %}"
...
The `process_view` method will be called whenever our custom middleware is invoked. If the request is an HTTP PUT request, it will parse the request body into a `QueryDict` object and store it into the `POST` attribute of the request object. Now modify the list of middleware in `simple-todo/simple_todo/simple_todo/settings.py` like this:
```python
MIDDLEWARE = [
    "layout.middleware.HttpPutMiddleware",
...
If you try submitting the form now, you will see the following error message in the terminal which the development server is running in:
```
Method Not Allowed (PUT): /todo_lists/2/info/
[11/Dec/2025 03:24:35] "PUT /todo_lists/2/info/ HTTP/1.1" 405 0
```

This happens because performing a redirect doesn't change the HTTP method being used and our todo list info view doesn't support HTTP PUT. We can fix this by simply adding a `put` method to our todo list info view which returns the value returned by calling `get` with the same parameters `put` received. Open `simple-todo/simple_todo/todo_lists/views.py` and modify it like this:
```python
from django.contrib.auth.mixins import LoginRequiredMixin
from django.db.utils import IntegrityError
from django.shortcuts import render
...
Now you should be able to successfully submit the todo list update form.
